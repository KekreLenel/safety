...
private
public :: sparse_grid_cc, sparse_grid_cfn_size
...
!
subroutine abscissa_level_closed_nd( level_max, dim_num, test_num, test_val, test_level )

  integer, intent(in)  ::  level_max, dim_num, test_num, test_val(dim_num,test_num) 
  integer, intent(out) ::  test_level(test_num)
  integer :: j, order, t
...
end subroutine abscissa_level_closed_nd
...
!    1 <= I <= ORDER.
!
!    Output, real(dp) :: CC_ABSCISSA, the value of the I-th 
...
!
!    Output, real(dp) :: W(N), the weights of the rule.
!
...
function index_to_level_closed ( dim_num, t, order, level_max )
  ...
  integer, intent(in) :: dim_num, t(dim_num), order, level_max
  integer :: dim
  ...
  return
end function index_to_level_closed
...
subroutine level_to_order_ccs ( dim_num, level, order )
  ...
  integer, intent(in) :: dim_num, level(dim_num)
  integer, intent(out) :: order(dim_num)
  ...
  return
end subroutine level_to_order_ccs
...
subroutine level_to_order_closed ( dim_num, level, order )
  ...
  return
end subroutine level_to_order_closed
...
subroutine levels_closed_index ( dim_num, level_max, point_num, grid_index )
  ...
  integer, intent(in) :: dim_num, point_num, level_max
  integer, intent(out) :: grid_index(dim_num,point_num)
...
!
  ...
  return
end subroutine levels_closed_index
...
!
!    Output, real(dp) :: VALUE, the value of the integral of the
!    monomial.
!
subroutine monomial_int01 ( dim_num, expon, value )

  integer, intent(in) :: dim_num, expon(dim_num)
  real(dp), intent(out) :: value

  value = 1.0D+00 / real ( product ( expon(1:dim_num) + 1 ), kind = 8 )

  return
end subroutine monomial_int01
...
!
!    Input, real(dp) :: WEIGHT(POINT_NUM), the quadrature weights.
!
!    Input, real(dp) :: X(DIM_NUM,POINT_NUM), the quadrature points.
!
!    Output, real(dp) :: QUAD_ERROR, the quadrature error.
!
subroutine monomial_quadrature ( dim_num, expon, point_num, weight, x, quad_error )

  integer, intent(in) :: dim_num, expon(dim_num), point_num
  real(dp), intent(in) :: weight(point_num), x(dim_num,point_num)
  real(dp), intent(out) :: quad_error

  real(dp) :: exact
  real(dp) :: quad
  real(dp) :: scale
  real(dp) :: value(point_num)
!
...
!
  ...
  return
end subroutine monomial_quadrature
...
!
!    Input, real(dp) :: X(DIM_NUM,POINT_NUM), the point coordinates.
!
!    Input, integer :: EXPON(DIM_NUM), the exponents.
!
!    Output, real(dp) :: VALUE(POINT_NUM), the value of the monomial.
!
...
!
!    Output, real(dp) :: W_ND(DIM_NUM,ORDER_ND), the product rule weights.
!
  implicit none
...
!
  ...
  real(dp) :: factor_value(factor_order)
  integer :: j
  integer :: k
  integer, save :: rep
  integer, save :: skip
...
!    as determined by SPARSE_GRID_Cfn_SIZE. 
! 
!    Output, real(dp) :: GRID_WEIGHT(POINT_NUM), the weights. 
! 
!    Output, real(dp) :: GRID_POINT(DIM_NUM,POINT_NUM), the points. 
! 

subroutine sparse_grid_cc ( dim_num, level_max, point_num, grid_weight, grid_point )
  ...
  real(dp) :: grid_point(dim_num,point_num)
  real(dp) :: grid_weight(point_num)
  integer :: level_max
  integer :: order_max
  integer :: point
! 
...
!
!    The necessary dimensions of GRID_INDEX can be determined by 
!    calling SPARSE_GRID_Cfn_SIZE first.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!
  ...
  integer, allocatable, dimension ( :, : ) :: grid_index2
  real(dp) :: grid_weight(point_num)
  real(dp), allocatable, dimension ( : ) :: grid_weight2
  integer :: h
  integer :: level
  integer :: level_1d(dim_num)
  integer :: level_max
