...
!
  ...
  do dim = 1, dim_num
    ...
    do level_idx = 0, level_max
     
      level = level_idx
      call level_to_order_hermite ( 1, level, order )

      allocate ( points(1:order(1)) )

      call hermite_compute_points ( order(1), points )

      do point = 1, point_num
        if ( sparse_order(dim,point) == order(1) ) then
...
!
  ...
  do level_idx = level_min, level_max
!
!  The middle loop generates a GRID,
!  based on the next partition that adds up to LEVEL.
!
    ...
    do
      ...
      call comp_next ( level_idx, dim_num, level_1d, more_grids, h, t )
  ...
  do dim = 1, dim_num
    ...
    do level_idx = 0, level_max
      ...
      do point = 1, point_total_num
        if ( sparse_total_order(dim,point) == order(1) ) then
...
!
  ...
  integer :: level_min
  logical more_grids
  integer :: point_total_num, order_1d(dim_num)
  integer :: t
!
!  Special case.
!
...
!
  ...
  integer :: dim
  integer :: h
  integer :: level_idx, level(1), order(1)
  integer :: level_1d(dim_num)
  integer :: level_max
  integer :: level_min
  logical more_grids
...
!
    ...
    do
      ...
      level = level_idx
      call comp_next ( level(1), dim_num, level_1d, more_grids, h, t )
